...
public interface KnowledgeBaseMapper extends BaseMapper<KnowledgeBase> {
    ...
    /** 我的知识库分页（含作者昵称与模块/文章计数） */
    @Select("<script>" +
            "SELECT b.id, b.kb_no AS kbNo, b.name, b.kb_type AS kbType, b.summary, b.cover_url AS coverUrl, " +
            "       b.status, b.current_version AS currentVersion, b.subscribe_count AS subscribeCount, " +
            ...
            "WHERE b.deleted = 0 AND b.author_id = #{authorId} " +
            "<if test='status != null'>AND b.status = #{status}</if>" +
            " ORDER BY b.updated_at DESC LIMIT #{offset}, #{size}" +
            "</script>")
    List<Map<String, Object>> selectMyPage(@Param("authorId") Long authorId,
                                           @Param("status") Integer status,
                                           @Param("offset") long offset,
                                           @Param("size") int size);

    @Select("<script>" +
            "SELECT COUNT(*) FROM knowledge_base b WHERE b.deleted = 0 AND b.author_id = #{authorId} " +
            "<if test='status != null'>AND b.status = #{status}</if>" +
            "</script>")
    long countMyPage(@Param("authorId") Long authorId, @Param("status") Integer status);
